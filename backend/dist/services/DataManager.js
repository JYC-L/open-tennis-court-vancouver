...
class AvailabilityManager {
    ...
    async getAvailability(clubName, date, startDate, endDate, requestedAt) {
        ...
        try {
            await this._withTimeout(this.orchestrator.onDemandUpdate(requestedAt, startDate, endDate), timeoutWindow, `Orchestrator timed out after ${timeoutWindow / 1000} seconds.`);
            records = await this.courtScheduleRepository.getAllAvailabilityAsArr();
        }
        catch (err) {
    ...
    async updateDatabase(clubName, court, date, startDate, endDate, data, lastUpdated) {
        await this.collection.updateOne({ clubName, court, date, startDate, endDate }, { $set: { ...data, lastUpdated } }, { upsert: true });
    }
}
