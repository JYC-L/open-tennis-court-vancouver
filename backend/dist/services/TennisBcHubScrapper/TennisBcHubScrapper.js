...
var _TennisBcHubScrapper_instances, _TennisBcHubScrapper_makeEndpointPattern;
const { launch } = require("puppeteer");
const { makecourtBookingLink } = require("./makecourtBookingLink.js");
class TennisBcHubScrapper {
    constructor() {
        _TennisBcHubScrapper_instances.add(this);
        this.locationInfo = {
            richmond: {
                courtName: "Richmond",
    ...
    async getCourtBooking() {
        ...
        for (const { locKey, resources } of allResults) {
            if (!Array.isArray(resources))
                continue; // Defensive: skip if not an array
    ...
    async interceptURLPrams() {
        ...
        for (const locKey of Object.keys(this.locationInfo)) {
            ...
            const endpoint_pattern = __classPrivateFieldGet(this, _TennisBcHubScrapper_instances, "m", _TennisBcHubScrapper_makeEndpointPattern).call(this, resourceFingerprint);
...
_TennisBcHubScrapper_instances = new WeakSet(), _TennisBcHubScrapper_makeEndpointPattern = function _TennisBcHubScrapper_makeEndpointPattern(resource_fingerprint) {
...
async function scrape(endpoint_pattern, resource_fingerprint) {
    ...
    await page.goto(`https://clubspark.ca/${resource_fingerprint}/Booking/bookbycourt`, { waitUntil: "networkidle2" });
    let targetUrl;
    try {
        targetUrl = await Promise.race([
            targetPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("Target request not found")), 1000))
        ]);
    }
    catch (error) {
        await browser.close();
