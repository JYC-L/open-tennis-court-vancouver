...
class UeTennisScrapper {
    ...
    async queryAvailability() {
        ...
        for (const [courtNumber, serviceId] of Object.entries(this.serviceIds)) {
            try {
                const response = await this.sendPostRequestsForAvailability(serviceId, startDateStr, endDateStr);
    ...
    async writeAvailabilityDataToMem(EntriesResponseArr) {
        ...
        EntriesResponseArr.forEach((entry) => {
            ...
            const courtName = Object.entries(this.serviceIds).find(([key, value]) => value === serviceId)?.[0];
    ...
    async sendPostRequestsForAvailability(serviceId, startDate, endDate) {
        try {
            const response = await axios.post("https://www.uetennis.com/_api/availability-calendar/v1/availability/query", {
                anyResource: false,
    ...
    async writeJsonToDisk(filename = "availability.json") {
        ...
        fs.writeFileSync(filePath, JSON.stringify(this.allAvailability, null, 2), "utf8");
